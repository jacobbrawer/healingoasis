...
# LED configuration
LED_COUNT = 10  # Reduced for testing
...
class PolarH10:
    def __init__(self, device):
        ...
        try:
            self.pixels = neopixel_spi.NeoPixel_SPI(SPI_DEVICE, LED_COUNT, pixel_order=neopixel_spi.GRB)
    ...
    def update_calibration(self, t):
        ...
        if elapsed_time <= self.CALIBRATION_DURATION:
            ...
            if target_led_count != self.last_active_leds:
                breath_phase = "Inhale" if phase < 0.5 else "Exhale"
                print(f"\r{breath_phase:6s}: {'▇' * (target_led_count // (LED_COUNT // 10))}{' ' * ((LED_COUNT - target_led_count) // (LED_COUNT // 10))} {target_led_count:3d}", end="")
    ...
    def finish_calibration(self):
        ...
        print("Switching to dynamic breath monitoring (red LEDs).")
    ...
    def update_leds(self, color):
        if self.pixels is None:
            print("NeoPixel SPI not initialized. Skipping LED update.")
            return

        try:
            if self.active_leds != self.last_active_leds:
                print(f"Updating LEDs: active={self.active_leds}, color={color}")
                self.pixels.fill((0, 0, 0))  # Turn off all LEDs
                for i in range(self.active_leds):
                    self.pixels[i] = color
                self.pixels.show()
                self.last_active_leds = self.active_leds
                print("LED update completed successfully")
        except Exception as e:
